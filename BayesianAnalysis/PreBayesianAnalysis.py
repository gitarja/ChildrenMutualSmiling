...
if __name__ == '__main__':
    ...
    for segment in segments:
        ...
        for n_model in [SAMPLES_VI]:
            ...
            plt.savefig(BAYESIAN_RESULTS_PPC_PATH + "\\pre_ppc_"+label+".pdf", format='pdf', transparent=True, bbox_inches='tight')
            plt.close()
            ...
            plt.savefig(BAYESIAN_RESULTS_VI_PATH + "\\pre_vi_"+label+".pdf", format='pdf', transparent=True, bbox_inches='tight')
            plt.close()

            # save features importance
            with open(BAYESIAN_RESULTS_MODEL_PRE_PATH + "\\" + "pre_features_BART_VI_" + label + ".pkl", 'wb') as handle:
                VI_summary = vi_results
                cpkl.dump(VI_summary, handle, protocol=cpkl.DEFAULT_PROTOCOL)
                # save the model
            with open(BAYESIAN_RESULTS_MODEL_PRE_PATH + "\\" + "pre_features_BART_" + label + ".pkl", 'wb') as handle:
                print("write data into: " + "pre_features_BART_" + label + ".pkl")
                cpkl.dump(idata, handle, protocol=cpkl.DEFAULT_PROTOCOL)
            with open(BAYESIAN_RESULTS_MODEL_PRE_PATH + "\\" + "pre_features_X_" + label + ".pkl", 'wb') as handle:
                print("write data into: " + "pre_features_X_" + label + ".pkl")
                cpkl.dump(X, handle, protocol=cpkl.DEFAULT_PROTOCOL)
            with open(BAYESIAN_RESULTS_MODEL_PRE_PATH + "\\" + "pre_features_y_" + label + ".pkl", 'wb') as handle:
            ...
            with open(BAYESIAN_RESULTS_MODEL_PRE_PATH + "\\" + "pre_features_mu_" + label + ".pkl", 'wb') as handle:
